...
class WiFiManagerParameter {
  public:
    ...
    */
    ...
    WiFiManagerParameter(const WiFiManagerParameter& rhs);
    WiFiManagerParameter& operator=(const WiFiManagerParameter& rhs);
    WiFiManagerParameter(WiFiManagerParameter&& rhs) noexcept;
    WiFiManagerParameter& operator=(WiFiManagerParameter&& rhs) noexcept;
    ~WiFiManagerParameter();
...
class WiFiManager
{
  public:
    ...
    // enable OTA update endpoint (disabled by default)
    void          setEnableUpdate(boolean enabled);

    // set OTA update basic auth credentials
    void          setUpdateCredentials(const char* username, const char* password);
  ...
  protected:
    ...
    boolean       _enableUpdate           = false; // register ota endpoints
    boolean       _updateAuth             = false; // require auth on ota endpoints
    boolean       _enableApi              = true;  // lightweight json api
    ...
    uint32_t      _otaTimeoutBackup       = 0xFFFFFFFF; // sentinel to restore portal timeout after ota
