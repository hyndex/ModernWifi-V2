...
WiFiManagerParameter::WiFiManagerParameter(const WiFiManagerParameter& rhs){
  init(rhs._id, rhs._label, rhs._value ? rhs._value : "", rhs._length, rhs._customHTML, rhs._labelPlacement);
}
...
WiFiManagerParameter::WiFiManagerParameter(WiFiManagerParameter&& rhs) noexcept {
  _id             = rhs._id;
  _label          = rhs._label;
  _labelPlacement = rhs._labelPlacement;
  _customHTML     = rhs._customHTML;
  _length         = rhs._length;
  _value          = rhs._value;
  rhs._value      = nullptr;
  rhs._length     = 0;
...
WiFiManagerParameter& WiFiManagerParameter::operator=(WiFiManagerParameter&& rhs) noexcept {
  if(this != &rhs){
    if(_value != nullptr){
      delete[] _value;
    }
...
void WiFiManager::setupHTTPServer(){
  ...
  if(_enableApi){
    server->on(F("/api/status"), HTTP_GET, std::bind(&WiFiManager::handleApiStatus, this));
    server->on(F("/api/scan"), HTTP_GET, std::bind(&WiFiManager::handleApiScan, this));
    server->on(F("/api/connect"), HTTP_POST, std::bind(&WiFiManager::handleApiConnect, this));
    server->on(F("/api/config"), HTTP_ANY, std::bind(&WiFiManager::handleApiConfig, this));
  }
...
boolean  WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  if(!_configPortalIsBlocking){
    ...
    #endif
    return true; // portal started successfully
  }
...
String WiFiManager::getMenuOut(){
  ...
  for(auto menuId :_menuIds ){
    ...
    if((String)_menutokens[menuId] == "update" && !_enableUpdate) continue; // skip update menu when ota disabled
...
void WiFiManager::handleApiStatus(){
  ...
  String body = F("{\"connected\":");
  body += isConnected ? F("true") : F("false");
  body += F(",\"ssid\":\""); body += jsonEscape(WiFi_SSID()); body += F("\"");
  body += F(",\"ip\":\""); body += isConnected ? WiFi.localIP().toString() : F(""); body += F("\"");
  body += F(",\"ap\":\""); body += jsonEscape(_apName); body += F("\"");
  body += F(",\"hostname\":\""); body += jsonEscape(getWiFiHostname()); body += F("\"");
  body += F(",\"portalActive\":"); body += configPortalActive ? F("true") : F("false");
  body += F(",\"apClients\":"); body += WiFi_softap_num_stations();
  body += F(",\"fw\":\""); body += jsonEscape(String(FPSTR(S_brand)) + " " + VER_ARDUINO_STR); body += F("\"}");
  server->sendHeader(FPSTR(HTTP_HEAD_CORS), FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));
  server->send(200, F("application/json"), body);
}

void WiFiManager::handleApiScan(){
  if(captivePortal()) return;
  handleRequest();
  bool force = server->hasArg(F("refresh"));
  WiFi_scanNetworks(force,false);
  if(_numNetworks < 0){
    server->sendHeader(FPSTR(HTTP_HEAD_CORS), FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));
    server->send(500, F("application/json"), F("{\"error\":\"scan failed\"}"));
    return;
  }
  String body = "[";
  for (int i = 0; i < _numNetworks; i++) {
    if(i) body += ',';
    int32_t enc = WiFi.encryptionType(i);
    bool secure = enc != (int32_t)WM_WIFIOPEN;
    body += F("{\"ssid\":\""); body += jsonEscape(WiFi.SSID(i)); body += F("\",");
    body += F("\"strength\":"); body += WiFi.RSSI(i); body += ',';
    body += F("\"secure\":"); body += secure ? F("true") : F("false"); body += ',';
    body += F("\"channel\":"); body += WiFi.channel(i);
    body += F(",\"latency\":"); body += 0; // placeholder until device reports latency
    body += "}";
  }
  body += "]";
  server->sendHeader(FPSTR(HTTP_HEAD_CORS), FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));
...
void WiFiManager::handleApiConnect(){
  if(captivePortal()) return;
  handleRequest();
  ...
  if(ssid == "" || ssid.length() > 32){
    server->sendHeader(FPSTR(HTTP_HEAD_CORS), FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));
    server->send(400, F("application/json"), F("{\"error\":\"invalid ssid\"}"));
    return;
  }
  ...
  _ssid = ssid;
  _pass = pass;
  uint8_t res = connectWifi(_ssid, _pass, true);
  bool ok = res == WL_CONNECTED;
  String body = F("{\"connected\":");
  body += ok ? F("true") : F("false");
  body += F(",\"status\":\""); body += jsonEscape(getWLStatusString(res)); body += F("\"");
  body += F(",\"ip\":\""); body += ok ? WiFi.localIP().toString() : F(""); body += F("\"");
  body += "}";
  server->sendHeader(FPSTR(HTTP_HEAD_CORS), FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));
  server->send(ok ? 200 : 500, F("application/json"), body);
}
...
void WiFiManager::handleApiConfig(){
  ...
  if(server->method() == HTTP_GET){
    String body = F("{\"hostname\":\""); body += jsonEscape(_hostname); body += F("\",");
    body += F("\"portalTimeout\":"); body += (_configPortalTimeout/1000); body += F(",");
    body += F("\"captive\":"); body += _enableCaptivePortal ? F("true") : F("false"); body += F(",");
    body += F("\"apSsid\":\""); body += jsonEscape(_apName); body += F("\",");
    body += F("\"apPasswordSet\":"); body += (_apPassword.length() > 0 ? F("true") : F("false"));
    body += F("}");
    server->sendHeader(FPSTR(HTTP_HEAD_CORS), FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));
    server->send(200, F("application/json"), body);
    return;
  ...
  if(server->hasArg(F("hostname"))){
    setHostname(server->arg(F("hostname")).c_str());
  }
  if(server->hasArg(F("portalTimeout"))){
    unsigned long secs = server->arg(F("portalTimeout")).toInt();
    setConfigPortalTimeout(secs);
  }
  if(server->hasArg(F("captive"))){
    _enableCaptivePortal = server->arg(F("captive")) == F("true") || server->arg(F("captive")) == F("1");
  }
  if(server->hasArg(F("apSsid"))){
    _apName = server->arg(F("apSsid"));
  }
  if(server->hasArg(F("apPassword"))){
    _apPassword = server->arg(F("apPassword"));
    if(!validApPassword()){
      server->sendHeader(FPSTR(HTTP_HEAD_CORS), FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));
      server->send(400, F("application/json"), F("{\"error\":\"ap password invalid\"}"));
      return;
    }
  }
...
void WiFiManager::handleWifiSave() {
  ...
  bool ssidMissing  = _ssid.length() == 0;
  bool ssidTooLong  = _ssid.length() > 32;
  bool passTooShort = _pass.length() > 0 && _pass.length() < 8;
  bool passTooLong  = _pass.length() > 63;

  if(ssidMissing || ssidTooLong || passTooShort || passTooLong){
    String page = getHTTPHead(FPSTR(S_titlewifisettings), FPSTR(C_wifi));
    page += F("<h3>Invalid WiFi credentials</h3><p>");
    page += F("SSID must be 1-32 characters. ");
    page += F("Password must be empty for open networks or 8-63 characters for secured networks.</p>");
    if(_showBack) page += FPSTR(HTTP_BACKBTN);
    page += getHTTPEnd();
    HTTPSend(page);
    _ssid = "";
...
void WiFiManager::handleInfo() {
  ...
  if(_showInfoUpdate && _enableUpdate){
...
/**
 * toggle OTA update endpoint availability
 * @param boolean enabled
 */
void WiFiManager::setEnableUpdate(boolean enabled){
  _enableUpdate = enabled;
}

/**
 * set OTA update basic auth credentials
 * setting credentials enables auth on update routes
 */
void WiFiManager::setUpdateCredentials(const char* username, const char* password){
  _updateUsername = username != nullptr ? username : "";
  _updatePassword = password != nullptr ? password : "";
  _updateAuth     = _updateUsername.length() || _updatePassword.length();
}
...
String WiFiManager::jsonEscape(const String& value) {
  String out;
...
bool WiFiManager::ensureUpdateAuth(){
  if(!_updateAuth) return true;
  if(server->authenticate(_updateUsername.c_str(), _updatePassword.c_str())) return true;
  server->requestAuthentication(HTTPAuthMethod::BASIC_AUTH);
  return false;
}

void WiFiManager::beginOtaUpload(){
  if(_otaTimeoutBackup == 0xFFFFFFFF){
    _otaTimeoutBackup = _configPortalTimeout;
  }
  _configPortalTimeout = 0; // keep portal alive during upload
}

void WiFiManager::endOtaUpload(){
  if(_otaTimeoutBackup != 0xFFFFFFFF){
    _configPortalTimeout = _otaTimeoutBackup;
    _otaTimeoutBackup = 0xFFFFFFFF;
  }
}
